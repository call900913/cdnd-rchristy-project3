      ...
      steps {
        dir("/var/lib/jenkins/workspace/cloud-rchristy-project3_master") {
          sh '''
            ...
            kubectl apply -f udacity-c3-deployment/kubernetes/env-configmap.yaml
            kubectl apply -f udacity-c3-deployment/kubernetes/env-secret.yaml
            kubectl apply -f udacity-c3-deployment/kubernetes/aws-secret.yaml
            kubectl apply -f udacity-c3-deployment/kubernetes/frontend.yaml
            kubectl apply -f udacity-c3-deployment/kubernetes/reverseproxy.yaml
            kubectl apply -f udacity-c3-deployment/kubernetes/backend-feed.yaml
            kubectl apply -f udacity-c3-deployment/kubernetes/backend-user.yaml
            '''
        }
      }
    }
